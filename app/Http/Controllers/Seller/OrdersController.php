...
class OrdersController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('seller.content.orders.index');
    }
    ...
    public function ajaxData($column = null, $operator = null)
    {
        ...
        $DataTables = DataTables::of(
            Order::query()->with(['user', 'user_address', 'status', 'orderProduct'])->where($column, $operator)
                ->whereHas('orderProduct',function($q){
                    return $q->where('seller_id',\auth()->guard('seller')->id());
        ...
        return created_at_filter($DataTables)->editColumn('created_at', function (Order $order) {
            ...
            ->addColumn('order_status', function (Order $order) {
                ...
                return [
                    ['url' => route('seller.orders.show', $order), 'icon' => 'eye'],
                ];
            })
            ->only([
    ...
    public function create()
    {
        ...
        return view('seller.content.orders.create')->with($data);
    }
    ...
    public function viewOrderPrice($id)
    {
        ...
        return view('seller.content.orders.priceOrder.show-order',compact('order','totalPrices','statuses'));
    }

    public function show($id)
    {

        $order = Order::with(['user','products','orderProduct'=>
            function ($q){
                return $q->where('seller_id',\auth()->guard('seller')->id());
            }])->find($id);
        ...
        return view('seller.content.orders.show-order',
            compact('order','totalPrices','statuses'));
...
//            }
//        }
//        return view('seller.content.orders.show-order', compact('order', 'total_price', 'results', 'main_settings'));

//        return view('seller.content.orders.show-order',compact());
    ...
    public function edit($id)
    {
        ...
        return view('seller.content.orders.edit')->with($data);
    }
    ...
    public function show_bundle($id, $bundle_id)
    {
        ...
        return view('seller.content.orders.showBundle', compact('bundle', 'products', 'order'));
    }
    ...
    public function updateOrderPrice(Request $request,$order)
    {
        ...
        try {
            ...
            $Order->update(['status_id'=>$request->input('status'),'viewed_at'=>now(),'total'=>$totalPrices]);
            ...
            Toaster::Success('Order Date Saved Successfully','success');
        }catch (\Exception $e){
