...
use App\Imports\ProductsImportSheet;
...
{
    private const ProductView = 'seller.content.products.';
//    private $authSeller;
...
//            $DataTables = Product::query();
    ...
    public function sheetFile()
    {
        $locale = app()->getLocale();
//        $file = base_path("sheets/products-${locale}.xlsx");
//        if (!is_file($file)){
//            $file = base_path("sheets/products-en.xlsx");
//        }
    ...
    public function importShow()
    {
        ...
        $branches = Branch::query()->where('seller_id', auth()->id())->get();

        return view(self::ProductView.'import', compact('categories', 'brands', 'manufacturers', 'branches'));
    }
    ...
    public function data($kind = '')
    {
        ...
        return datatables()->of($query)
            ...
            ->addColumn('options', function (Product $product) {
                $back = "";
                $back .= '<a href="' . route('seller.products.show', $product->id) . '" class="btn waves-effect waves-light btn-outline-warning" title="edit">Show</a>';
                $back .= '&nbsp;<a href="' . route('seller.products.attribute.index', $product->id) . '" class="btn waves-effect waves-light btn-outline-success" title="inactive">Attribute</a>&nbsp;';
                $back .= '&nbsp;<a href="' . route('seller.products.edit', $product->id) . '" class="btn waves-effect waves-light btn-outline-info" title="edit">Edit</a>&nbsp;';
                $back .= Form::open(['url' => route('seller.products.destroy', $product->id), 'class' => 'd-inline', 'onclick' => 'return confirm("Are you sure?")']);
                if ($product->on_sale == 1 || $product->is_hot == 1) {
                    $back .= '&nbsp;<a href="' . route('seller.products.enddeal', $product->id) . '" class="btn waves-effect waves-light btn-outline-info" title="End Deal">End Deal</a>&nbsp;';
                }
                if ($product->is_active == 0) {
                    $back .= '&nbsp;<a href="' . route('seller.products.active', $product->id) . '" class="btn waves-effect waves-light btn-outline-success" title="active">Activate</a>&nbsp;';
                } else {
                    $back .= '&nbsp;<a href="' . route('seller.products.active', $product->id) . '" class="btn waves-effect waves-light btn-outline-danger" title="inactive">InActivate</a>&nbsp;';
                }
    ...
    public function create(Request $request)
    {
        ...
        return view(self::ProductView.'create.create', compact('issale', 'ishot', 'categories', 'brands', 'manufacturers', 'branches', 'attributes', 'combos'));
//        return view(self::ProductView.'_create', compact('issale', 'ishot', 'categories', 'brands', 'manufacturers', 'pattributes', 'combos'));
    ...
    public function getProductAttributes(Request $request, Product $product)
    {
        ...
        $attributes = $product::with('attributes')->find($id)
            ...
            ->pluck('name', 'id')
            ->unique();
        return view('seller.content.attributes.product_attributes')->with(compact('attributes'));
    }
    ...
    public function getCurrentProductAttributesAjax(Request $request, Product $product)
    {
        ...
        //dd($attributes);
        return view('seller.content.attributes.current_product_attributes')->with(compact('attributes'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $product->is_active = $request->has('is_active') == "on" ? 1 : 0;
            ...
            $product->stock = $request->get('stock') ?? 1;
...
//            }
            ...
            DB::rollBack();
            return $e->getMessage();
            return redirect()->route('seller.products.index')->with(['error'=>__('auth.errorNewSetting')]);
        }

        # assign options to product
        return redirect()->route('seller.products.index')->with(['success'=>__('auth.successNewSetting', ['var'=>__('layouts.product')])]);
    }
...
//        }
            ...
            $productBranch = BranchProduct::where('product_id', $product->id)->first();
            $productBranch->update(['branch_id'=>$request->get('branch_id')]);
            ...
            $logPayload = ['msg' => 'Product Updated', 'model_id' => $product->id, 'user_id' => auth()->id()];
            logToDatabase($logPayload);
            return redirect()->route('seller.products.index')->with(['success'=> __('auth.successUpdate', ['var'=> __('layouts.product')])]);
            ;
        } catch (\Exception $e) {
            return redirect()->route('seller.products.index')
                ->with(['error'=>__('auth.errorSetting')]);
        }
    }
...
//           $DataTables = Product::query()->where('on_sale',1);
            return $this->ajaxProductTable('on_sale', true);
        }
...
//            $DataTables = Product::query()->where('is_hot',1);
            return $this->ajaxProductTable('is_hot', true);
            //->addColumn('logo_thumb', '@isset($logo_thumb)<img src="{{$logo_thumb}}" style="width: 64px;" class="thumbnail" alt="">@endisset')->rawColumns(['logo_thumb'])
        }
...
//        $product->attributes()->attach($attributesValues);
    ...
    private function ajaxProductTable($column = null, $operator = null)
    {
//        $DataTables = DataTables::of($ProductDataTable->with([
//            'seller', 'categories', 'brand', 'manufacturer'
//        ])->orderByDesc('id'));

        $DataTables = DataTables::of(Product::query()->with([
                'seller', 'categories', 'brand', 'manufacturer','images'
            ])->where('seller_id', auth()->guard('seller')->id())->where($column, $operator)->orderByDesc('id'));
        ...
        return created_at_filter($DataTables)
            ...
            ->editColumn('created_at', function (Product $product) {
                ...
                return route('seller.products.edit', $product);
            })
            ->addColumn('delete_url', function (Product $product) {
                return route('seller.products.destroy', $product);
            })
            ...
            ->addColumn('actions', function (Product $product) {
                return [
                    [
                        'url' => route('seller.products.active', $product),
                        'icon' => $product->is_active ? 'x-circle' : 'check-circle',
                    ],
                    ['url' => route('seller.products.show', $product), 'icon' => 'eye'],
                ];
            })
