...
class BundlesController extends Controller
{
    ...
    {
        $products = Product::where('is_bundle', true)->orderBy('id', 'desc')->get();
        return view('admin.content.bundles.index', compact('products'));
    }
...
//  }
    ...
    public function create()
    {
        ...
        return view('admin.content.bundles.create', compact('categories', 'brands', 'manufacturers'));
    }

    public function store(Request $request)
    {
        $request->validate([
            'bundle_image' => 'required|mimes:jpeg,PNG,JPG,JPEG,jpg,png|max:10000'
        ]);
//        DB::beginTransaction();
        ...
        if (PrioritySetting::first()->enable == 1) {
            $order_id = \App\Models\Priority::where('name', 'bundle')->first()->order_id;
...
//        return $product;
        $coby_product = $product;
        unset($coby_product['name']);
        unset($coby_product['slug']);
        unset($coby_product['description']);

        $product = Product::create($coby_product);
        foreach ($this->languages as $local) {
            $productTrans = new ProductTranslation();
            $productTrans->product_id = $product->id;
            $productTrans->name = $request->input('name.' . $local->locale);
        ...
        $logPayload = ['msg' => 'Bundle Added', 'model_id' => $product->id, 'user_id' => auth()->id()];
...
//        return view('admin.content.bundles.show')->with( compact('order') );
//    }
    ...
    public function edit($id)
    {
        ...
        return view('admin.content.bundles.edit', compact('product', 'categories', 'brands', 'manufacturers', 'bundles', 'old_price', 'price'));
    }

    public function update(Request $request, $id)
    {
        $request->validate([
            'bundle_image' => 'mimes:jpeg,PNG,JPG,JPEG,jpg,png|max:10000'
        ]);
        $productsRuinedQuantity = BundleProduct::checkQuantity($request);
        if ($productsRuinedQuantity) {
            return back()->with('message', $productsRuinedQuantity);
        ...
        if (PrioritySetting::first()->enable == 1) {
            ...
            foreach ($request->products as $i => $id) {
                if (!priority($order_id, $id)) {
                    $sub_product = Product::find($id);
                    session()->flash('has_not_Priority', 'You canot add this offer "Bundle" has priority low with product "' . $sub_product->{'name:en'} . '"');
                    return back();
                }
            }
        }
        ...
        foreach ($request->products as $i => $id) {
            ...
            $bundle_product->discount = $request->discount[$i];
            $bundle_product->save();
            \App\Models\ProductCombo::where('product_id', $id)->delete();
            $sub_product = Product::find($id);
            $sub_product->on_sale = 0;
            $sub_product->is_hot = 0;
            $sub_product->save();
        ...
        # log the action to database
        $logPayload = ['msg' => 'Bundle Updated', 'model_id' => $product->id, 'user_id' => auth()->id()];
        logToDatabase($logPayload);
        return redirect()->route('admin.bundles.index');
    }
