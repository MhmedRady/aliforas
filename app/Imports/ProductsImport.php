       ...
       $manufacturer_id        = 36;
        ...
        if(isset($row[34]) && $row[34] != ''){
            ...
            foreach ($attrs as $attr) {
                ...
                $check_attr = AttributeTranslation::where('name',$attr)->where('locale','en')->first();
                if($check_attr){
                    $attr_id = $check_attr->attribute_id;
                    $check_exist = AttributeProduct::where('product_id',$product->id)->where('attribute_id',$attr_id)->first();
                    if($check_exist){
...
//        ]);
    ...
    private function getCategory($row) {
        ...
        $category_ar = CategoryTranslation::where('category_id',$category_id)->where('locale','ar')->first();
        $category_en = CategoryTranslation::where('category_id',$category_id)->where('locale','en')->first();
