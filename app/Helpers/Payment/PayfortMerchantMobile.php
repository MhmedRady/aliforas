...
class PayfortMerchantMobile extends PayfortBase {
    ...
    private function generateMerchantPageData()
    {
        ...
        $iframeParams = array(
            ...
            //'submit_url'=>$this->getBaseServiceURL()
        );
...
//                'device_fingerprint' => $params['device_fingerprint'],
        ...
        #$logsType = request()->session()->get('logsType');
        #$sessionId = request()->session()->get('sessionId');
        #PayfortLogs::logs($logsType,$sessionId,json_encode($postData),'','purchaseRequest');


        //calculate request signature
        $signature = $this->calculateSignature($postData, 'request');
        $postData['signature'] = $signature;
//        unset($postData['device_fingerprint']);
