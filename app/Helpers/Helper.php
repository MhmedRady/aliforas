...
class Helper
{
    public static function applyClasses()
    {
        // default data array
        $DefaultData = [
    ...
    {
        $cart = userCart()->getContent();
        $prices = [];
        foreach ($cart->all() as $item) {
            array_push($prices, ($item->price * $item->quantity));
        }
        $tax = MainSetting::query()->where('key', 'taxes')->first();
        $tax->value = (int)$tax->value;
        $sub_total = array_sum($prices);
        $taxes = $tax->value?(array_sum($prices) * ($tax->value/100)):0;
    ...
    public static function getTotalPricesApi(): array
    {
        $cart = apiUserCart()->getContent();
        $prices = [];

        foreach ($cart->all() as $item) {
            array_push($prices, ($item->last()->price * $item->quantity));
        }

        $tax = MainSetting::query()->where('key', 'taxes')->first();
        $tax->value = (int)$tax->value;
        $sub_total = array_sum($prices);
        $taxes = $tax ? (array_sum($prices) * ($tax->value/100)) : 0;
        $total = $sub_total + $taxes;
    ...
    public static function totalWeights()
    {
        $cart = userCart()->getContent();
        $weights = [];

        foreach ($cart->all() as $item) {
            array_push($weights, (ceil($item->associatedModel->weight??0) * $item->quantity));
        }
        return array_sum($weights);
    }

    public static function totalWeightsApi()
    {
        $cart = apiUserCart()->getContent();
        $weights = [];
    ...
    public static function getTotalOrderWithWeight($cod = 0, $fKG = 0, $addKG = 0): array
    {
        $prices   = self::getTotalPrices();
        $weights  = self::totalWeights();

        $sipping =$cod + $fKG + ($weights-1>0?(($weights-1) * $addKG):0);

        $total = round($sipping + $prices['total']);

        $prices['weight'] = $weights;
        $prices['sipping'] = $sipping;
        $prices['total'] = $total;

        return $prices;
    }

    public static function getTotalOrderWithWeightApi($cod = 0, $fKG = 0, $addKG = 0): array
    {
        $prices   = self::getTotalPricesApi();
        $weights  = self::totalWeightsApi();
    ...
    {
        if (is_array($roles)) {
            if (in_array(auth()->guard('admin')->user()->getRoleNames()->first(), $roles)) {
                return true;
            }
        } elseif (auth()->guard('admin')->user()->hasRole($roles)) {
            return true;
        } elseif ($roles === 'all') {
            return true;
        }
    ...
    public static function filterActive($word)
    {
        return (bool)preg_match('/^active/i', $word);
    }
}
