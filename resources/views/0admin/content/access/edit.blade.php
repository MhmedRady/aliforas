            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-body wizard-content ">
                        {!! Form::open(['url' => route('admin.roles.update', $row->id)]) !!}
                            {{method_field('PUT')}}
                            ...
                            <div class="form-group">
                                <select class="js-example-basic-single form-control" name="permissions[]" multiple="multiple">
                                    @foreach($permissions as $permission)
                                        @php
                                            $selected = ($row->permissions->find($permission->id)) ? 'selected' : '' ;
                                        @endphp
                            ...
                            <!-- <div class="form-group">
                                ...
                                {{ Form::text('order', $row->id, ['class' => 'form-control']) }}
                                @error('order')
                                <span class="error text-danger"> {{$message}} </span>
                                @enderror
                            </div> -->
