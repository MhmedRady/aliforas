            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-body wizard-content">
                        ...
                        {!! Form::open(["enctype" => "multipart/form-data", 'url' => route('admin.brands.update', $row->id), 'class' => 'tab-wizard vertical wizard-circle', 'id' => 'add_category_form']) !!}
                        {{ method_field('PATCH') }}
                        @foreach($languages as $locale)
                            ...
                            <section>
                                <div class="form-group">
                                    ...
                                    <input type="text" name="name[{{$locale->locale}}]" value="{{ $row->translate($locale->locale)->name ?? '' }}" class="form-control" required maxlength="40">
                                </div>
                            </section>
                        @endforeach
                        ...
                        <section>
                            ...
                            <div class="row">
                                <div class="col-md-2">
                                    <img class="thumbnail" style="width: 100px; height: 100px;" src="{{ $row->logo_url }}" alt="">
                                </div>
                                <div class="col-md-10">
                                    <div class="form-group">
                            ...
                            @foreach($languages as $locale)
                                ...
                                <section>
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="meta_title[{{$locale->locale}}]" value="{{ $row->translate($locale->locale)->meta_title ?? '' }}" class="form-control" maxlength="40">
                                    </div>

                                    <div class="form-group">
                                        <label for="meta_keywords[{{$locale->locale}}]">Meta Keywords {{$locale->name}} :</label>
                                        <input type="text" name="meta_keywords[{{$locale->locale}}]" value="{{ $row->translate($locale->locale)->meta_keywords ?? '' }}" class="form-control" maxlength="40">
                                    </div>

                                    <div class="form-group">
                                        <label for="meta_description[{{$locale->locale}}]">Meta Description {{$locale->name}} :</label>
                                        <textarea name="meta_description[{{$locale->locale}}]" rows="4" class="form-control" maxlength="160">{{ $row->translate($locale->locale)->meta_description ?? '' }}</textarea>
                                    </div>
                                </section>
                            @endforeach
