        ...
        <div class="row">
            <div class="col-12">
                <div class="card">
                    ...
                    {!! Form::open(['url' => route('admin.options.update', $row->id)]) !!}
                            {{ method_field('PUT') }}
                            ...
                            <div class="form-group">
                                {{ Form::label('field_id', 'Field') }}
                                <select name="field_id" id="field_id" class="form-control">
                                    <option value="">--select  value--</option>
                                    @foreach($fields as $k => $v)
                                        @php
                                            $showValues = ($v=='select'||$v=='radio'||$v=='checkbox') ? 1 : 0 ;
                                            $selected = ($k == $row->field_id) ? 'selected' : '' ;
                                        @endphp
                                        <option value="{{$k}}" data-showValues="{{$showValues}}" {{$selected}}>{{$v}}</option>
                                    @endforeach
                                </select>
                                @error('field_id')
                                    {{$message}}
                            ...
                            <div class="form-group" id="field_values" style="display:none">
                                <table class="table table-bordered table-striped" id="user_table">
                                    ...
                                    <tbody id="field_values_body">
                                        <tr>
                                            @foreach($row->values as $k => $v)
                                                    <td><input type="text" name="field_values[]" class="form-control" value="{{$v->value}}"/></td>
                                                @if($k == 0)
...
@section('page-script')
    ...
    <script>
        ...
        {
                html = '<tr>';
                html += '<td><input type="text" name="field_values[]" class="form-control" /></td>';
                if(number > 1) {
        ...
        function initFormValues(){
            ...
            var showValues = $('#field_id option:selected').attr('data-showValues');

            if(parseInt(showValues)){
                $('#field_values').show();
            } else {
                $('#field_values').hide();
            }
