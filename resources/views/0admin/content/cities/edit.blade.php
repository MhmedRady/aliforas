            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-body wizard-content">
                        ...
                        {!! Form::open(["enctype" => "multipart/form-data", 'url' => route('admin.cities.update', $row->id), 'class' => 'tab-wizard vertical wizard-circle', 'id' => 'add_category_form']) !!}
                        {{ method_field('PATCH') }}
                                ...
                                <select name="state_id" id="state_id" class="select2 form-control">
                                    @foreach($states as $state)
                                        <option value="{{ $state->id }}"
                                                @if($state->id == $row->state_id) selected @endif>{{ $state->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </section>
...
@section('page-style')
    ...
    <link rel="stylesheet" type="text/css"
          href="{{asset('admin-asset/assets/node_modules/select2/dist/css/select2.min.css')}}">
