            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-body wizard-content">
                        ...
                        {!! Form::open(["enctype" => "multipart/form-data",'url' => route('admin.dealsection.update', $row->id), 'class' => 'tab-wizard vertical wizard-circle', 'id' => 'add_category_form']) !!}
                        {{ method_field('PATCH') }}
                        @foreach($languages as $locale)
                            ...
                            <section>
                                <div class="form-group">
                                    ...
                                    <input type="text" name="name[{{$locale->locale}}]" class="form-control" required value="{{ $row->translate($locale->locale)->name ?? '' }}">
                                </div>
                                <div class="form-group">
                                    <label for="slug[{{$locale->locale}}]">slug {{$locale->name}} :</label>
                                    <input type="text" name="slug[{{$locale->locale}}]" class="form-control" required value="{{ $row->translate($locale->locale)->slug ?? '' }}">
                                </div>
                            </section>
                        @endforeach
                        ...
                        <section>
                            ...
                            <div class="form-group w-100">
                                ...
                                <select name="product_ids[]" class="form-control products select2 w-100" multiple>
                                    @if($products->count() > 0)
                                        @foreach($products as $product)
                                            <option value="{{$product->id}}" @if(in_array($product->id,explode(',',$row->product_ids))) selected @endif>{{$product->name}}</option>
                                        @endforeach
                                    @endif
                                </select>
                                @error('product_ids')
                            ...
                            <div class="btn-group" data-toggle="buttons">
                                <label class="btn btn-info active">
                                    <div class="custom-control custom-checkbox mr-sm-2">
                                        <input type="checkbox" class="custom-control-input" id="checkbox0" name="is_active" @if($row->is_active == 1) checked @endif>
                            ...
                            <div class="btn-group" data-toggle="buttons">
                                <label class="btn btn-info active">
                                    <div class="custom-control custom-checkbox mr-sm-2">
                                        <input type="checkbox" class="custom-control-input" id="in_header" name="is_home" @if($row->is_home == 1) checked @endif>
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="discount" class="form-control" value="{{$row->discount}}" required>
                            </div>
