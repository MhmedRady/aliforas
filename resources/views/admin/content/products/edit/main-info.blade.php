<div id="product-type-modern" class="content" role="tabpanel" aria-labelledby="product-type-modern-trigger">
        ...
        <div class="mb-1 col-md-6">
            ...
            <select class="form-select" name="brand_id" id="brand_id">
                ...
                @foreach($brands as $id => $brand)
                    <option value="{{ $brand->id }}" {{($row->brand->id ?? old('brand_id'))==$brand->id?'selected':''}}>{{ $brand->name }}</option>
                @endforeach
            </select>
        </div>
    </div>
    ...
    <div class="row">
        ...
        <div class="mb-1 col-md-4">
            ...
            <input type="number" id="weight" class="form-control" value="{{old('weight')??$row->weight}}" name="weight">
        </div>
        <div class="mb-1 col-md-4">
            <label class="form-label" for="length">length</label>
            <input type="number" id="length" class="form-control" value="{{old('length')??$row->length}}" name="length">
        </div>
    ...
    <div class="row">
        <div class="mb-1 col-md-4">
            <div class="d-flex flex-column">
                ...
                <div class="form-check form-switch form-check-success" style="margin-left: 35px">
        ...
        <div class="mb-1 col-md-4">
            ...
            <input type="number" id="height" class="form-control" value="{{old('height')??$row->height}}" name="height">
        </div>

        <div class="mb-1 col-md-4">
            <label class="form-label" for="width">width</label>
            <input type="number" id="width" class="form-control" value="{{old('width')??$row->width}}" name="width">
        </div>
    </div>
