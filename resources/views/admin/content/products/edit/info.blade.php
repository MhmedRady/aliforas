        ...
        @foreach($languages as $locale)
            <div class="mb-1 col-md-6">
                ...
                <input type="text" name="name[{{$locale->locale}}]" class="form-control required"
                       value="{{ old("name." . $locale->locale)??($row->translate($locale->locale)->name??'')}}" maxlength="40">
            </div>
        @endforeach
    </div>
    <div class="row">
        @foreach($languages as $locale)
            ...
            <div class="mb-1 form-password-toggle col-md-6">
                ...
                <textarea id="description[{{$locale->locale}}]" name="description[{{$locale->locale}}]" class="form-control summernote" cols="30" rows="10">
                    {!! (old("description." . $locale->locale) ?? ($row->translate($locale->locale)->description??'')) !!}
                </textarea>
            </div>
        @endforeach
    </div>
