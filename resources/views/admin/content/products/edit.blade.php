                    ...
                    <div class="card-body wizard-content">
                        ...
                        @foreach($languages as $locale)
                            ...
                            <section>
                                <div class="form-group">
                                    ...
                                    <input type="text" name="name[{{$locale->locale}}]"
                                           value="{{ $row->translate($locale->locale)->name ?? '' }}"
                                           class="form-control" maxlength="40">
                                </div>

                                <div class="form-group">
                                    <label for="slug[{{$locale->locale}}]">Slug {{$locale->name}} :</label>
                                    <input type="text" name="slug[{{$locale->locale}}]"
                                           value="{{ $row->translate($locale->locale)->slug ?? '' }}"
                                           class="form-control" maxlength="40">
                                </div>

                                <div class="form-group">
                                    <label for="description[{{$locale->locale}}]">Description {{$locale->name}}
                                        :</label>
                                    <textarea id="description[{{$locale->locale}}]"
                                              name="description[{{$locale->locale}}]" class="form-control summernote"
                                              cols="30"
                                              rows="10">{{ $row->translate($locale->locale)->description ?? '' }}</textarea>
                                </div>
                            </section>
                        @endforeach
                        ...
                        <section>
                            ...
                            <div class="form-group">
                                ...
                                <input min="0" type="number" step="1" class="form-control required" id="stock"
                                       name="stock" required value="{{ $row->stock ?? '' }}">
                            </div>

                            <div class="form-group">
                                <label for="minimum_stock">Low Stock Notification</label>
                                <input min="0" type="number" step="1" class="form-control required" id="minimum_stock"
                                       name="minimum_stock" required value="{{ $row->minimum_stock ?? '' }}">
                            </div>

                            <div class="form-group">
                                <label for="price">Price</label>
                                <input min="0" type="number" class="form-control required" id="price" name="price"
                                       required value="{{ $row->price }}">
                            </div>

                            <div class="form-group">
                                <label for="reward_points">Reward Points</label>
                                <input type="number" class="form-control required" id="price" min="0"
                                       value="{{ $row->reward_points ?? '' }}" name="reward_points">
                            </div>
                            ...
                            <div class="form-group" data-show="return_allowed" style="display: none">
                                ...
                                <input min="0" type="number" step="1" class="form-control" id="return_duration"
                                       name="return_duration" value="{{ $row->return_duration }}">
                            </div>

                            <div class="form-group">
                                <label for="on_sale">
                                    <input type="checkbox" id="on_sale" name="on_sale"
                                           @if($row->on_sale) checked @endif >
                                    On sale?
                                </label>
                            </div>
                            ...
                            <div class="form-group" data-show="on_sale" style="display: none">
                                ...
                                <input min="0" type="number" class="form-control" id="before_price" name="before_price"
                                       value="{{ $row->before_price }}">
                            </div>

                            <div class="form-group" data-show="on_sale" style="display: none">
                                <label for="sale_ends_at">On Sale ends at</label>
                                <input type="text" class="form-control" id="sale_ends_at" name="sale_ends_at"
                        ...
                        <!--                            <div class="form-group" style="{{$row->is_hot?'display:none':'display:none'}}">
                                <label for="is_combo">
                            ...
                            <div class="form-group" data-show="is_hot" style="@if(!$row->is_hot) display: none @endif">
                                <label for="hot_starts_at">Hot starts at</label>
                                <input type="text" class="form-control" id="hot_starts_at" name="hot_starts_at"
                                       value="{{ $row->hot_starts_at }}">
                            </div>

                            <div class="form-group" data-show="is_hot" style="@if(!$row->is_hot) display: none @endif">
                                <label for="hot_ends_at">Hot ends at</label>
                                <input type="text" class="form-control" id="hot_ends_at" name="hot_ends_at"
                                       value="{{ $row->hot_ends_at }}">
                            </div>

                            <div class="form-group" data-show="is_hot" style="@if(!$row->is_hot) display: none @endif">
                                <label for="hot_price">New Price</label>
                                <input min="0" type="number" class="form-control" id="hot_price" name="hot_price"
                                       value="{{ $row->hot_price }}">
                            </div>

                        <!--                            <div class="form-group" data-show="is_combo" style="display: none">
                                <label class="mdb-main-label">Combo</label>
                                <select class="form-control" multiple="multiple" searchable="Search here.." id="combos"
                                        name="combo_id[]" style="width: 100%;">
                                    &lt;!&ndash; <option value="" disabled selected>select combo</option> &ndash;&gt;

                                    @foreach($combos as $combo)
                            <option
                                value="{{$combo->id}}" {{in_array( $combo->id , $row->getComboIdsAttribute())? 'selected' : ''}}>{{ app()->getLocale()=='ar'? $combo->name_ar: $combo->name_en}}</option>
                                    @endforeach
                        ...
                        <section>
                            ...
                            <script id="gallery-item" type="text/html">
                                <li>
                                    ...
                                    <input type="file" accept="image/*" name="images[]" class="file"/>
                                    <input type="hidden" name="thumbnail[]" class="thumb-input" value="0"/>
                                    <div class="options">
                                        <a href="#" class="set-thumbnail"><i
                                                class="mdi mdi-checkbox-blank-circle-outline"></i> Thumbnail</a>
                                ...
                                <ul>
                                    @foreach($row->images as $image)
                                        <li @if($image->image == $row->thumbnail) class="selected" @endif >
                                            <img class="thumbnail" src="{{ $image->image_url }}" alt="">
                                            <input type="hidden" name="images~[]" value="{{ $image->id }}">
                                            <input type="hidden" name="thumbnail[]" class="thumb-input"
                                                   value="{{ $image->image == $row->thumbnail ? 1 : 0 }}"/>
                                            <div class="options">
                                                @if($image->image == $row->thumbnail)
                                                    <a href="#" class="set-thumbnail"><i
                                                            class="mdi mdi-check-circle-outline"></i> Thumbnail</a>
                                                @else
                                                    <a href="#" class="set-thumbnail"><i
                                                            class="mdi mdi-checkbox-blank-circle-outline"></i> Thumbnail</a>
                                                @endif
                        ...
                        <section>
                            ...
                            <div class="form-group">
                                ...
                                <select name="category_id" id="category_id" class="form-control"
                                        onchange="attributesDependOnCategory(this)">
                                    {{--<option id="default" selected
                                            value="{{ $row->get_category()->id }}">{{ $row->get_category()->name }}</option>--}}
                                    @foreach($categories as $id => $name)
                                        @if(substr_count($name, '-') < substr_count(next($categories), '-') )
                                            <option disabled value="{{ $id }}"
                                                    style="color: #f70000">{{ $name }}</option>
                                        @else
                                ...
                                <select name="brand_id" id="brand_id" class="form-control">
                                    @foreach($brands as $brand)
                                        <option value="{{ $brand->id }}"
                                                @if($brand->id == $row->brand_id) selected @endif>{{ $brand->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                                ...
                                <select name="manufacturer_id" id="manufacturer_id" class="form-control">
                                    @foreach($manufacturers as $manufacturer)
                                        <option value="{{ $manufacturer->id }}"
                                                @if($manufacturer->id == $row->manufacturer_id) selected @endif>{{ $manufacturer->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            {{-- <div class="form-group">
                                ...
                                <select name="show_for" id="show_for" class="form-control">
                                    @foreach(['male', 'female', 'both'] as $for)
                                        <option value="{{ $for }}"  @if($for == $row->show_for) selected @endif>
                                            {{ $for }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="btn-group">
                                <label class="btn btn-info active">
                                    <div class="custom-control custom-checkbox mr-sm-2">
                                        <input type="checkbox" class="custom-control-input" @if($row->is_active) checked
                                               @endif id="checkbox0" name="active">
                            ...
                            <div class="btn-group">
                                <label class="btn btn-info active">
                                    <div class="custom-control custom-checkbox mr-sm-2">
                                        <input type="checkbox" class="custom-control-input" @if($row->is_point) checked
                                               @endif  id="checkbox00" name="point">
                                        <label class="custom-control-label" for="checkbox00">Point</label>
                                    </div>
                                </label>
                            </div>
                            ...
                            @foreach($languages as $locale)
                                ...
                                <section>
                                    <div class="form-group">
                                        <label for="meta_title[{{$locale->locale}}">Meta Title {{$locale->name}}
                                            :</label>
                                        <input type="text" name="meta_title[{{$locale->locale}}]"
                                               value="{{ $row->translate($locale->locale)->meta_title ?? '' }}"
                                               class="form-control" maxlength="40">
                                    </div>

                                    <div class="form-group">
                                        <label for="meta_keywords[{{$locale->locale}}]">Meta Keywords {{$locale->name}}
                                            :</label>
                                        <input type="text" name="meta_keywords[{{$locale->locale}}]"
                                               value="{{ $row->translate($locale->locale)->meta_keywords ?? '' }}"
                                               class="form-control" maxlength="40">
                                    </div>

                                    <div class="form-group">
                                        <label for="meta_description[{{$locale->locale}}]">Meta
                                            Description {{$locale->name}} :</label>
                                        <textarea name="meta_description[{{$locale->locale}}]" rows="4"
                                                  class="form-control"
                                                  maxlength="160">{{ $row->translate($locale->locale)->meta_description ?? '' }}</textarea>
                                    </div>
                                </section>
                            @endforeach
        ...
        $(".tab-wizard").steps({
            ...
            onStepChanging: function (event, currentIndex, newIndex) {
                ...
                if (currentIndex < newIndex && currentIndex == 3) {
                    if ($('#add_category_form-p-3 div.gallery img').length == 0) {
                        toastr.error('please insert one image at least');
                        return false;
                    }
                }
            ...
            onFinished: function (event, currentIndex) {
                ...
                var pictures = $(".attribute_pictures");
                var x = 0;
                ...
                for (var i = 0; i < pictures.length; i++) {
                    ...
                    if (pictures[i].files.length === 0 && !$(pictures.parent().parent()[i]).hasClass('attribute_row')) {
                ...
                if (quantity > stock) {
                    ...
                    $("#error").addClass("alert alert-danger").text("attribute picture is required");
                } else {
                    $('form').submit(function (ev) {
