                                ...
                                <ul class="row list-unstyled">
                                    ...
                                    @foreach($row->images as $image)
                                        <li @if($image->image == $row->thumbnail) class="selected col-3 m-2" @endif >
                                            <img class="img-thumbnail" src="{{ $image->image_url(150, 150) }}" alt="">
                        ...
                        <section>
                            ...
                            <div class="form-group">
                                <label for="category_id">Category</label>
                                 {{ ($row->categories[0]->name ?? ' - ') }} <p></p>
                                <label for="brand_id">Brand</label>
                                {{ $row->brand->name?? ' - ' }} <p></p>
                                <label for="manufacturer_id">Manufacturer</label>
                                 {{ $row->manufacturer->name ?? ' - ' }}  <p></p>

                                @if($row->seller)
                                    <label for="manufacturer_id">Seller Name</label>
                                    {{ $row->seller->name ?? 'Admin' }}  <p></p>

                                    <label for="manufacturer_id">Branch</label>
                                    {{ $row->branch()->first()->name?? ' - ' }}  <p></p>
                                @endif
                                <label for="show_for">Show For</label>
                                {{ ucfirst($row->show_for) }} <p></p>
