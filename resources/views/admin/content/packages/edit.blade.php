            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-body wizard-content">
                        ...
                        {!! Form::open(['url' => route('admin.packages.update', $row->id), 'class' => 'tab-wizard vertical wizard-circle', 'id' => 'add_package_form']) !!}
                        {{ method_field('PATCH') }}
                        @foreach($languages as $locale)
                            ...
                            <section>
                                <div class="form-group">
                                    ...
                                    <input type="text" name="name[{{$locale->locale}}]" class="form-control required" value="{{ $row->translate($locale->locale)->name ?? '' }}" required>
                                </div>

                                <div class="form-group">
                                    <label for="description[{{$locale->locale}}]">Description {{$locale->name}} :</label>
                                    <textarea id="description[{{$locale->locale}}]" name="description[{{$locale->locale}}]" class="form-control summernote required" required cols="30" rows="10">{{ $row->translate($locale->locale)->description ?? '' }}</textarea>
                                </div>
                            </section>
                        @endforeach
                        ...
                        <section>
                            ...
                            <div class="form-group">
                                ...
                                <input type="number" id="price" value="{{ $row->price ?? '' }}" name="price" class="form-control required" required>
                            </div>

                            <div class="form-group">
                                <label for="duration">Duration: </label>
                                <input type="number" id="duration" value="{{ $row->duration ?? '' }}" name="duration" class="form-control required" required>
                            </div>
                            ...
                            <div class="btn-group" data-toggle="buttons">
                                <label class="btn btn-info active">
                                    <div class="custom-control custom-checkbox mr-sm-2">
                                        ...
                                        <label class="custom-control-label" for="checkbox0">Package Active</label>
                                    </div>
                                </label>
                            </div>
                            ...
                            @foreach($languages as $locale)
                                ...
                                <section>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <textarea name="meta_keywords[{{$locale->locale}}]" value="{{ $row->translate($locale->locale)->meta_keywords ?? '' }}" rows="4" class="form-control"></textarea>
                                    </div>
                                </section>
                            @endforeach
