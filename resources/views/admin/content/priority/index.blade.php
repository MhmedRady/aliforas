                    ...
                    <div class="card-body">
                        <div class="custom-control custom-switch">
                            <input type="checkbox" class="custom-control-input" id="customSwitch1"
                                   onchange="toggole_priority()" {{$priority_setting->enable==1 ?'checked':''}}>
                            <label class="custom-control-label" for="customSwitch1">Enable Priority</label>
                        </div>
                            ...
                            <table id="myTable" class="table table-bordered table-striped">
                                ...
                                @foreach($rows as $row)
                                    <tr id="{{$row->id}}">
                                        ...
                                        <td>
                                            <a href="{{ route('admin.priority.edit', $row->id ) }}"
                                               class="btn btn-outline-primary"><i class="ti-pencil"></i> Edit </a>
                                        </td>
                                    </tr>
                                @endforeach
                                </tbody>
...
@section('page-script')
    ...
    <script>
        ...
        $(function () {
            var sorting = new Sortable(document.getElementById('tbody-id'), {
                ...
                onEnd: function (/**Event*/evt) {
                    ...
                    $.ajax({
                        url: '{{ route('admin.priority.updateall') }}',
                        data: {
                            ids: ids
                        },
                        type: "POST",
        ...
        function toggole_priority() {
            $.ajax({
                url: '{{ route('admin.priority.toggole') }}',
                type: "POST",
                headers: {
                    'X-CSRF-TOKEN': '{{ csrf_token() }}'
                },
