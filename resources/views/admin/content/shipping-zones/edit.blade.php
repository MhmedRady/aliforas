                ...
                <div class="card">
                    <div class="card-body wizard-content">
                        ...
                        {!! Form::open([
                            ...
                            'url' => route('admin.shipping_zones.update',$zone), 'class' => 'tab-wizard vertical wizard-circle', 'id' => 'add_brand_form']) !!}
                        @method('PUT')
                        <section>
                            <div class="form-group mb-2">
                                ...
                                <select name="company_id" class="select2 form-control">
                                    @foreach($companies as $company)
                                        <option value="{{$company->id}}" {{$zone->company->id == $company->id?'selected':''}}>{{$company->name}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </section>
                        <section>
                            <div class="form-group mb-2">
                                ...
                                <input type="text" name="zone_name" class="form-control" value="{{old('zone_name')??$zone->name}}">
                            </div>
                        </section>
                        <section>
                            <div class="form-group mb-2">
                                ...
                                <div class="mb-1">
                                    <select class="select2 form-select" name="areas[]" multiple="multiple" id="area" data-placeholder="Select Shipping Area">
                                        @foreach($areas as $area)
                                            <option value="{{$area->id}}" {{in_array($area->id, (old('areas')??$zone->areas)) ? 'selected="selected"':''}}>{{$area->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
...
{{--                                </select>--}}
                        ...
                        <section>
                            <div class="form-group mb-2">
                                ...
                                <input type="text" name="first_kg" class="form-control" value="{{old('first_kg')??$zone->first_kg}}">
                            </div>
                        </section>
                        <section>
                            <div class="form-group mb-2">
                                <label class="mb-1" for="additional_kg">Additional Kg:</label>
                                <input type="text" name="additional_kg" class="form-control" value="{{old('additional_kg')??$zone->additional_kg}}">
                            </div>
                        </section>
                        <section>
                            <div class="form-group mb-2">
                                <label class="mb-1" for="cod_values">Cod Values:</label>
                                <input type="text" name="cod_values" class="form-control" value="{{old('additional_kg')??$zone->cod_values}}">
                            </div>
                        </section>
