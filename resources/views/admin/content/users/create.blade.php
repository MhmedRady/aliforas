            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-body wizard-content">
                        ...
                        {!! Form::open(['url' => route('admin.users.store'), 'class' => 'tab-wizard vertical wizard-circle', 'id' => 'add_user_form']) !!}
                        ...
                        <section>
                            ...
                            <div class="form-group">
                                ...
                                {{ Form::select('role', $roles, null, ['placeholder'=>'Choose Role', 'class' => 'form-control'] ) }}
                            </div>
                                ...
                                <div class="d-block mt-1">
                                    @isset($orderstatus)
                                        @foreach($orderstatus as $status)
                                            <strong class="mr-2 ml-2">
                                                <label for="{{$status->name}}">{{$status->name}}</label>
                                                <input type="checkbox" id="{{$status->name}}" name="order_status_permissions[]" value="{{$status->id}}">
                                            </strong>
                                        @endforeach
                                    @endisset
                                </div>
                        ...
                        <button type="submit" class="btn btn-primary btn-md btn-block mt-3 mb-3">Submit</button>
