...
@section('tableColumns')
    ...
    <th>{{__('auth.Email')}}</th>
    <th>{{__('auth.phoneNumber')}}</th>
    <th>{{__('layouts.products')}}</th>
    @if(currentRoute() == 'seller.orders.index')
    <th data-options="{{ \App\Models\OrderStatus::all()->mapWithKeys(fn($orderStatus) => [$orderStatus->id => $orderStatus->name]) }}"
        class="select-search">
        @lang('layouts.orderStatus')
    </th>
    @else
        <th>@lang('layouts.orderStatus')</th>
    @endif
@endsection
@include('seller.table.ajaxTable-inline', [
    'url' => request()->url(),
    'title' => __('layouts.orders'),
    'columns' => ['user_title', 'user_email', 'user_phone', 'items_count', 'order_status'],
])
@endsection
        ...
        <div class="row page-titles">
            ...
            <div class="col-md-7 align-self-center text-right">
                <div class="d-flex justify-content-end align-items-center">
                    ...
                    <a href="{{route('admin.orders.create')}}" class="btn btn-info d-none d-lg-block m-l-15"><i
                                    ...
                                    <!-- <td>{{$order->shipped_at}}</td> -->
                                        <td>
                                            ...
                                            <a href="{{route('admin.orders.show', $order->id)}}"
                                               class="btn waves-effect waves-light btn-outline-warning" title="edit">Show</a>
