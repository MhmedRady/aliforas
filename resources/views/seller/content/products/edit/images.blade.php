...
<div id="images-details-modern" class="content text-start" role="tabpanel" aria-labelledby="images-details-modern-trigger">
            ...
            <ul>
                @foreach($row->images as $image)
                    <li @if($image->image == $row->thumbnail) class="selected" @endif >
                        <img class="thumbnail" src="{{ $image->image_url(250,250) }}" alt="">
                        ...
                        <div class="options">
                            @if($image->image == $row->thumbnail)
                                <a href="{{$image->image_url(250,250)}}" class="set-thumbnail" target="_blank"><i
                                        class="mdi mdi-check-circle-outline"></i> Thumbnail</a>
                            @else
                                <a href="{{$image->image_url(250,250)}}" class="set-thumbnail" target="_blank">
                                    <i class="mdi mdi-checkbox-blank-circle-outline"></i> Thumbnail</a>
                            @endif
                            <a href="#" class="delete-gallery"><i class="delete mdi mdi-delete"></i></a>
                        </div>
                        <i class="icon mdi mdi-check-circle-outline"></i>
