      ...
      @if(!Auth::guard('web')->check())
        <form class="theme-form" action="{{route("web.userAuth")}}" method="post">
          ...
          <div class="form-group">
            ...
            <input type="text" class="form-control" id="email" name="Email" required="" vlaue="{{old("Email")}}">
            @error('Email')
          ...
          <div class="form-group">
            <label for="review">{{__("home.Password")}}</label>
            <input type="password" class="form-control" id="review" name="Password" required="" vlaue="{{old("Password")}}">
            @error('Password')
            <div id="EmailFeedback" class="invalid-feedback font-85 d-block">
              <strong>{{$message}}</strong>
            </div>
          ...
          <a href="{{route("web.resetPassword")}}" class="d-block mb-3"><h5>{{__("Login.Forgot Your Password?")}}</h5></a>
          <div class="form-group">
          ...
          <div class="account-fwd">
            <a href="{{route("web.user-register")}}" class="d-block">
              <h6>{{__("Login.Need An Account?")}}</h6>
            </a>
          </div>
        </form>
      @else
        <div class="user-account-details container">
          <div class="row">
            ...
            <div class="col-6 username-box m-auto">
              <span>{{userNameSplit(Auth::User()->name)}}</span>
            </div>
            ...
            <div class="form-group">
              <a href="{{route('web.user-profile')}}" class="btn btn-info btn-md btn-block">
            ...
            <div class="form-group">
              <a href="{{route("web.view-change-password")}}" class="btn btn-warning btn-md btn-block">
                  <i class="fa fa-key" aria-hidden="true"></i>
                {{__("auth.changePass")}}
              </a>
            </div>
            ...
            <div class="form-group">
              <a href="{{route("web.userLogout")}}" class="btn btn-primary btn-md btn-block ">
