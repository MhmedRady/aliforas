    ...
    <div class="container-fluid">
        ...
        <div class="row page-titles">
            <div class="col-md-5 align-self-center">
                <h4 class="text-themecolor">{{__("seller.Pages")}}</h4>
            </div>
            <div class="col-md-7 align-self-center text-right">
                <div class="d-flex justify-content-end align-items-center">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="javascript:void(0)">{{__("seller.Home")}}</a></li>
                        <li class="breadcrumb-item "><a href="javascript:void(0)">{{__("seller.Categories")}}</a></li>
            ...
            <div class="col-12">
                <div class="card">
                    ...
                    @foreach($languages as $locale)
                           <h5 class="text-info">{{$locale->name}} {{__("seller.content")}}</h5>
                            <section>
                                <div class="form-group">
                                    <label for="name[{{$locale->locale}}]"> {{__("seller.n")}} {{$locale->name}} :</label>
                                     ...
                                     <label for="description[{{$locale->locale}}]">{{__("seller.Description")}} {{$locale->name}} :</label>
                        ...
                        @endforeach
                            ...
                            <div class="form-group">
                                ...
                                <label for="price">{{__("seller.Price")}}</label>
                                {{ $row->price }}<p></p>
                                <label for="reward_points">Reward Points</label>
                                {{ $row->reward_points ?? '' }}<p></p>
                                <label for="reward_points">{{__("seller.ToReturn")}}</label>
                                @if($row->return_allowed) {{__("seller.Y")}} @else {{__("seller.N")}} @endif<p></p>
                                <label for="reward_points">{{__("seller.RDuration")}}</label>
                                {{$row->return_duration}}<p></p>
                                <label for="reward_points">{{__("seller.pOnSale")}}?</label>
                                @if($row->on_sale) {{__("seller.Y")}} @else {{__("seller.N")}} @endif<p></p>
                                <label for="reward_points">{{__("seller.BPriceBDiscount")}}</label>
                                {{ $row->before_price }}<p></p>
                                <label for="is_hot">{{__("seller.isHotP")}}?</label>
                                @if($row->is_hot) {{__("seller.Y")}} @else {{__("seller.N")}} @endif<p></p>
                                <!--<label for="is_hot">Is Combo product?</label>
                                @if($row->is_combo) Y{{__("seller.Y")}} @else {{__("seller.N")}} @endif<p></p>-->
                                <label for="hot_starts_at">{{__("seller.HotStartsAt")}}</label>
                                {{ $row->hot_starts_at }}<p></p>
                                <label for="hot_ends_at">{{__("seller.HotEndAt")}}</label>
                                {{ $row->hot_ends_at }}<p></p>
                                <label for="hot_price"> {{__("seller.nPrice")}} </label>
                                {{ $row->hot_price }}<p></p>
                            </div>
                        ...
                        <section>
                            ...
                            <h5 class="text-info">{{__("seller.imgList")}}</h5>
                            <div class="gallery">
                                <ul class="row list-unstyled">

                                    @foreach($row->images as $image)
                                        <li @if($image->image == $row->thumbnail) class="selected col-3 m-2" @endif >
                                            <img class="img-thumbnail" src="{{ thumb('product', 150, 150, $image->image) }}" alt="">
                                            ...
                                            <div class="options">
                                                @if($image->image == $row->thumbnail)
                                                    <a href="#" class="set-thumbnail"><i class="mdi mdi-check-circle-outline"></i> {{__("seller.Thumbnail")}} </a>
                                                @else
                                                    <a href="#" class="set-thumbnail"><i class="mdi mdi-checkbox-blank-circle-outline"></i> {{__("seller.Thumbnail")}} </a>
                                                @endif
                        ...
                        <section>
                            ...
                            <h6 class="text-info">Attributes</h6>
                                ...
                                @foreach($attributes as $attr)
                                 ...
                                 @if($attr->group_id != "")
                                     <div class="recordset">
                                        @foreach ($get_attributes_value as $attrib)
                                            <div class="row">
                                                ...
                                                <div class="col-2">
                                                    ...
                                                    @php
                                                        $product_attribute_image=json_decode($attrib->picture);
                                                    ...
                                                    <div class="d-block">
                                                        ...
                                                        @if (!is_null($attrib->picture))
                                                            @foreach($product_attribute_image as $img)
                                                                <img class="img-thumbnail d-block" src="{{ URL::to('/upload/products/') }}/{{$img}}" style="width:150px; height:50px;">
                                                            @endforeach    
                                                        @endif
