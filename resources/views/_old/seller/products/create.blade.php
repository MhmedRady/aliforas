            ...
            <div class="col-md-7 align-self-center text-right">
                <div class="d-flex justify-content-end align-items-center">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">{{__("sellerProducts")}}</li>
                    </ol>
                    <a href="{{route('seller.products.import.show')}}" class="btn btn-cyan btn-sm d-block m-l-15" style='margin:5px'><i class="fa fa-plus-circle"></i> {{__("seller.importProducts")}} </a>
                ...
                <div class="card">
                    <div class="card-body wizard-content">
                        ...
                        {!! Form::open([
                            ...
                            'url' => route('seller.products.store'), 'class' => 'tab-wizard vertical wizard-circle', 'id' => 'add_category_form']) !!}
                        ...
                        @foreach($languages as $locale)
                            ...
                            <section>
                                <div class="form-group">
                                    ...
                                    <input type="text" name="name[{{$locale->locale}}]" class="form-control {{$locale->locale =='ar'?'required':''}}" value="{{ old("name." . $locale->locale) }}" {{$locale->locale =="ar"?"required":""}} >
                                </div>
                        ...
                        <h6>Stock & Branch</h6>
                        <section>
                          @include('seller.products.accordians.stock')
                        </section>

                        <h6>Images</h6>
                        <section>
                        @include('seller.products.accordians.images')
                        </section>

                        <h6>Meta Content</h6>
                        <section>
                        @include('seller.products.accordians.main-info')
                        </section>

                        <h6>Attributes</h6>
                        <section>
                        @include('seller.products.accordians.attributes')
...
@section('scripts')
    ...
    var count = 1;
        ...
        function attributesDependOnCategory(ele)
        {
            ...
            $.ajax({
                url: '{{ url('seller/attributes-product') }}',
                type: "POST",
                headers: {
                    'X-CSRF-TOKEN': '{{ csrf_token() }}'
                },
        ...
        function currentAttributesDependOnCategory(ele)
        {
            ...
            //console.log(cat_ids);
            $.ajax({
                url: '{{ url('seller/attributes-current-product') }}',
                type: "POST",
                headers: {
                    'X-CSRF-TOKEN': '{{ csrf_token() }}'
                },
