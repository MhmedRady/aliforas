...
@section('container')
    ...
    <section class="container ">
        ...
        <div class="mb-3 bg-white p-3">
            <form action="{{route('seller.products.attribute.store',$product)}}" enctype="multipart/form-data" class="" method="post">
                @csrf
        ...
        <div class="container bg-white table-responsive">
            <table id="table_data" class="table table-hover">
                ...
                @foreach($product->attributes as $item)
                    <form class="d-none" id="form_delete_{{$loop->index}}" action="{{route('seller.products.attribute.destroy',['product'=>$product,'attribute'=>$item])}}" method="post">
                        @method('delete')
                        @csrf
                    </form>
                    <form method="post" enctype="multipart/form-data" action="{{route('seller.products.attribute.update',['product'=>$product,'attribute'=>$item])}}">
                        @csrf
                        @method('put')
                        <tr>
                            <td>{{$item->name}}</td>
